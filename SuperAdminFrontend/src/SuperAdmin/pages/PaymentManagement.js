...
const TABLE_HEAD = [
  ...
  { id: 'serviceprovider', label: 'Service Provider', alignLeft: true },
...
export default function PaymentManagement() {
  ...
  useEffect(() => {
    ...
    const getUserdata = async () => {
      const reqData = await fetch("http://localhost:5000/api/payments");
              ...
              <Table>
                ...
                <TableBody>
                  {userData.map((row, index) => {
                    const { tenantname, amount, serviceprovider, date, status } = row;
