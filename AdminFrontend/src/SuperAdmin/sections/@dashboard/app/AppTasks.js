...
export default function AppTasks({ title, subheader }) {
  ...
  const [tasks, setTasks] = useState(() => {
    const savedTasks = localStorage.getItem('superAdminTasks');
  ...
  useEffect(() => {
    localStorage.setItem('superAdminTasks', JSON.stringify(tasks));
  }, [tasks]);
    ...
    <Card>
      ...
      >
        <Stack p={2} spacing={2}>
          ...
          />
          <Button variant="contained" className='bg-blue-500' onClick={handleAddTask}>Add Task</Button>
        </Stack>
      </Popover>
